...
# Video generation imports (lazy-loaded to avoid crashes if not installed)
try:
...
try:
    ...
    from moviepy.video.io import ffmpeg_wrappers
    HAS_MOVIEPY = True
except ImportError:
    try:
...
# Create a 'db' namespace so existing app code (db.init_db, db.stats, etc.) works
class _DBNamespace:
    init_db = staticmethod(init_db)
    stats = staticmethod(stats)
    get_news = staticmethod(get_news)
    insert_news = staticmethod(insert_news)
    update_news_status = staticmethod(update_news_status)
    update_news_category = staticmethod(update_news_category)
...
page = st.sidebar.radio(
    "Navigation",
    ["📊 Dashboard", "📡 News Feed", "✍️ Post Editor", "🎬 Video Studio",
     "📅 Scheduled Posts", "📤 Publish History", "⚙️ Settings"],
...
# Color schemes for video backgrounds
VIDEO_THEMES = {
    "Breaking News (Red)": {"bg": (180, 20, 20), "text": (255, 255, 255), "accent": (255, 220, 50)},
    "Dark Professional": {"bg": (20, 25, 40), "text": (240, 240, 250), "accent": (80, 180, 255)},
    "Clean White": {"bg": (245, 245, 250), "text": (30, 30, 40), "accent": (200, 40, 40)},
    "CG Saffron": {"bg": (255, 153, 51), "text": (0, 0, 0), "accent": (0, 0, 128)},
    "Forest Green": {"bg": (20, 60, 30), "text": (240, 255, 240), "accent": (180, 255, 100)},
}
...
def _create_news_frame(headline, subtitle, theme_colors, size=(1080, 1920)):
    """Create a single news frame image using PIL."""
    if not HAS_PIL:
        return None
    W, H = size
    img = Image.new("RGB", (W, H), color=theme_colors["bg"])
    draw = ImageDraw.Draw(img)

    # Top bar
    bar_h = 120
    draw.rectangle([0, 0, W, bar_h], fill=theme_colors["accent"])
    label_font = _get_font(48, bold=True)
    draw.text((40, 30), "📰 BREAKING NEWS", fill=theme_colors["bg"], font=label_font)

    # Headline
    head_font = _get_font(56, bold=True)
    max_w = W - 80
    head_lines = _wrap_text(headline, head_font, max_w, draw)
    y = bar_h + 100
    for line in head_lines[:8]:
        draw.text((40, y), line, fill=theme_colors["text"], font=head_font)
        y += 70

    # Subtitle / source
    if subtitle:
        sub_font = _get_font(36)
        sub_lines = _wrap_text(subtitle, sub_font, max_w, draw)
        y += 40
        for line in sub_lines[:4]:
            draw.text((40, y), line, fill=theme_colors["accent"], font=sub_font)
            y += 50

    # Bottom bar
    draw.rectangle([0, H - 80, W, H], fill=theme_colors["accent"])
    bot_font = _get_font(32, bold=True)
    draw.text((40, H - 60), "Chhattisgarh News Hub", fill=theme_colors["bg"], font=bot_font)

    return img


def _create_end_frame(theme_colors, size=(1080, 1920)):
    """Create end card frame."""
    if not HAS_PIL:
        return None
    W, H = size
    img = Image.new("RGB", (W, H), color=theme_colors["bg"])
    ...
    draw.text((W//2 - 200, H//2 - 60), "Follow Us", fill=theme_colors["text"], font=font)
    draw.text((W//2 - 250, H//2 + 20), "@ChhattisgarhNewsHub", fill=theme_colors["accent"], font=sub_font)
    return img


def generate_news_video(headline, content, theme_name="Breaking News (Red)",
                        use_ai_voice=True, lang="hi", output_path=None):
    """Generate a news video with optional AI voiceover.
    Returns path to the generated video file.
    """
    if not HAS_PIL:
        return {"ok": False, "msg": "Pillow not installed. Run: pip install Pillow"}
    if not HAS_MOVIEPY:
        return {"ok": False, "msg": "moviepy not installed. Run: pip install moviepy"}

    theme = VIDEO_THEMES.get(theme_name, VIDEO_THEMES["Breaking News (Red)"])
    tmpdir = tempfile.mkdtemp()

    try:
        # Generate AI voice if requested
        audio_path = None
        voice_text = f"{headline}. {content}"
        if use_ai_voice:
            if not HAS_GTTS:
                return {"ok": False, "msg": "gTTS not installed. Run: pip install gTTS"}
        ...
        if audio_path:
            ...
            duration = max(10, min(60, len(content) / 10))

        # Create frames
        frames = []

        # Headline frame (first 40% of duration)
        head_img = _create_news_frame(headline, "", theme)
        head_path = os.path.join(tmpdir, "frame_head.png")
        head_img.save(head_path)
        head_clip = ImageClip(head_path, duration=duration * 0.4)
        frames.append(head_clip)

        # Content frame (next 40%)
        content_img = _create_news_frame("", content[:200], theme)
        content_path = os.path.join(tmpdir, "frame_content.png")
        content_img.save(content_path)
        content_clip = ImageClip(content_path, duration=duration * 0.4)
        frames.append(content_clip)

        # End frame (last 20%)
        end_img = _create_end_frame(theme)
        end_path = os.path.join(tmpdir, "frame_end.png")
        end_img.save(end_path)
        end_clip = ImageClip(end_path, duration=duration * 0.2)
        frames.append(end_clip)
        ...
        return {"ok": False, "msg": f"Video generation error: {e}"}
...
# ===========================================================================
    ...
    st.markdown("Create news videos with AI voiceover — for YouTube Shorts, Instagram Reels, Facebook.")
    st.markdown("---")

    # Check dependencies
    dep_cols = st.columns(4)
    with dep_cols[0]:
        st.metric("Pillow", "✅" if HAS_PIL else "❌")
    with dep_cols[1]:
        st.metric("gTTS (Voice)", "✅" if HAS_GTTS else "❌")
    with dep_cols[2]:
        st.metric("MoviePy", "✅" if HAS_MOVIEPY else "❌")
    with dep_cols[3]:
        st.metric("FFmpeg", "✅" if os.system("which ffmpeg > /dev/null 2>&1") == 0 else "❌")

    if not HAS_PIL or not HAS_MOVIEPY:
        st.markdown("---")
        st.warning("⚠️ Video dependencies not fully installed on this server.")
        st.info("To enable video generation, add these to requirements.txt:\nPillow\ngTTS\nmoviepy")
    ...
    vid_type = st.radio(
        "Select Video Type",
        ["🎬 Slideshow Video (Text + Background)",
         "🎙️ AI Voice News Video (Text + Hindi Voice)"],
        label_visibility="collapsed",
    )

    use_voice = "AI Voice" in vid_type

    st.markdown("### 📝 News Content")

    # Pre-fill from session state (if coming from news feed)
    default_headline = st.session_state.get("edit_title", "")
    default_content = st.session_state.get("edit_summary", "")

    v_headline = st.text_input("Video Headline", value=default_headline,
                                placeholder="Enter news headline for the video...")
    v_content = st.text_area("News Content (will be spoken in video)",
                             value=default_content, height=120,
                             placeholder="Enter the full news content...")

    st.markdown("### 🎨 Video Style")
    vs1, vs2 = st.columns(2)
    with vs1:
        v_theme = st.selectbox("Theme", list(VIDEO_THEMES.keys()))
    with vs2:
        if use_voice:
            v_lang = st.selectbox("Voice Language", ["hi (Hindi)", "en (English)"], index=0)
            v_lang_code = v_lang[:2]
        else:
            v_lang_code = "hi"
            st.info("📝 Text-only video (no voice)")

    # Preview frame
    if v_headline and HAS_PIL:
        st.markdown("### 👀 Frame Preview")
        theme = VIDEO_THEMES.get(v_theme, VIDEO_THEMES["Breaking News (Red)"])
        preview_img = _create_news_frame(v_headline[:60], v_content[:80] if v_content else "", theme)
        if preview_img:
            import io as _io
            buf = _io.BytesIO()
            preview_img.save(buf, format="PNG")
            st.image(buf.getvalue(), caption="Preview (headline frame)", use_column_width=True)

    st.markdown("---")

    # Generate button
    if st.button("🎬 Generate Video", type="primary", use_container_width=True,
                 disabled=not v_headline):
        if not v_headline:
            st.warning("Please enter a headline.")
        elif not HAS_PIL or not HAS_MOVIEPY:
            st.error("Video libraries not installed. See message above.")
        else:
            with st.spinner("Generating video... This may take 1-2 minutes."):
                result = generate_news_video(
                    headline=v_headline,
                    content=v_content or v_headline,
                    theme_name=v_theme,
                    use_ai_voice=use_voice,
                    lang=v_lang_code,
                )

            if result.get("ok"):
                st.success(f"✅ Video generated! Duration: {result.get('duration', 0):.0f}s")
                # Show video player
    ...
    st.markdown("---")
    st.caption("💡 Tip: Videos are generated in 9:16 vertical format (1080x1920) — perfect for "
               "YouTube Shorts, Instagram Reels, and Facebook Stories.")
